...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/04 10:25:15 by abismail          #+#    #+#             */
/*   Updated: 2025/08/06 10:27:10 by a-khairi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_env_valid(char *args)
{
	...
	while (args[i])
	{
		if (!((ft_isdigit(args[i]) && i != 0) || (ft_isalpha(args[i]))
				|| args[i] == '_'))
		{
...
int	is_exist(t_env *env, const char *envi)
{
	...
	{
		len = ft_strlen(envi);
		if (ft_strncmp(env->env, envi, len) == 0 && ((env->env[len] == '=')
				|| !env->env[len]))
			return (1);
		env = env->next;
	}
