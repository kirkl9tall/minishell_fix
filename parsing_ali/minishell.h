...
/*   Updated: 2025/08/04 10:17:35 by abismail         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int						handle_plain_text_token(int *i, char *line,
							char **token);
int						append_part(char **token, char *part);
int						only_spaces(const char *s);
char					*varible_ex(t_shell *shell, int *i, char **token,
							int *flag);
char					*randomize(void);
int						check_ambiguous(char *token, t_random *rend, int *flag);
char					**split_by_custom_separator(const char *str,
							const char *sep, int *count, int *capacity);
...
int						handle_double_quote_content(int *i, t_shell *shell,
							char **token);
int						handle_dq_token_part(int *i, t_shell *shell,
							char **token);
char					*replace_spaces_with_sep(const char *str,
							const char *sep);
char					*ft_getenv(t_env *env, const char *key);
void					add_back_env(t_env *env, char *var);
int						is_empty_token_with_only_spaces(char **token,
							char *part, t_shell *shell, int k);
int						is_fully_empty_case(char **token, char *part,
							t_shell *shell, int k);
int						is_null_token_with_empty_quotes(char **token,
							char *part, t_shell *shell, int k);
...
void					handle_quoted_delimter(char *line, int *i,
							char **token);
char					*get_delimiter(char *line, int *i);
int						count_words(const char *str);
char					*extract_var_part(t_shell *shell, int *i);
int						add_token_to_list(t_token_node **head, char *token,
							int type, int amb);
t_redirect				*create_redirect_node(char *file, int append,
							int *flag);
...
int						red_two(t_shell *shell, t_command *cmd,
							t_redirect *analyser);
...
int						red_one(t_shell *shell, t_command *cmd,
							t_redirect *analyser);
int						red_zero(t_shell *shell, t_command *cmd,
							t_redirect *analyser);
int						redirection_mode(t_shell *shell, t_command *cmd,
							t_redirect *analyser);
...
int						is_it_alpha_valid(t_shell *shell, t_command *cmd,
							int i);
...
void					child_heredoc_exec(t_shell *shell, t_command *analyser,
							t_redirect *tmp);
int						type_three_heredoc(t_shell *shell, t_command *analyser,
							t_redirect *tmp);
void					none_equal_case_export(t_shell *shell, t_command *cmd,
							int i, t_env *trav);
void					export_case_equal(t_shell *shell, t_command *cmd, int i,
							t_env *trav);
void					exit_function(t_shell *shell, t_command *cmd);
